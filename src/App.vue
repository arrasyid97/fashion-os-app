...
const isSubscriptionActive = computed(() => {
    ...
    // Cek jika masa langganan berbayar aktif
    if (status === 'active' && userProfile.data.subscriptionEndDate) {
        // Cek jika subscriptionEndDate adalah Firestore Timestamp atau JS Date
        const endDateValue = userProfile.data.subscriptionEndDate.seconds 
            ? userProfile.data.subscriptionEndDate.seconds * 1000 
            : new Date(userProfile.data.subscriptionEndDate).getTime();

        if (endDateValue > now) return true;
    }
...
async function saveMasterHargaSupplier() {
    ...
    try {
        ...
        state.produk.forEach(p => {
            const productRef = doc(db, "products", p.docId);
            // Simpan hargaBeli ke database
            batch.update(productRef, { hargaBeli: p.hargaBeli || 0 });
        });

        await batch.commit();
        alert("Katalog harga supplier berhasil diperbarui!");
        hideModal();
    } catch (error) {
        console.error("Gagal simpan harga supplier:", error);
...
const searchCatalogQuery = ref(''); // Teks pencarian di modal

// Fungsi untuk memfilter produk berdasarkan SKU atau Nama Model
const filteredCatalog = computed(() => {
    if (!searchCatalogQuery.value) return state.produk;
    const query = searchCatalogQuery.value.toLowerCase();
    return state.produk.filter(p => 
        p.sku.toLowerCase().includes(query) || 
        (p.namaModel && p.namaModel.toLowerCase().includes(query))
    );
});
    ...
    try {
        ...
        await runTransaction(db, async (transaction) => {
            ...
            transaction.update(codeRef, {
                ...
                usedByEmail: userProfile.data.email, // <-- PERBAIKAN DI SINI
                usedAt: new Date()
            });
        });
...
async function executeCompleteTransaction() {
    ...
    // Menghitung rasio sisa harga setelah dipotong diskon voucher secara global
    const rasioBersih = summary.subtotal > 0 ? (summary.finalTotal / summary.subtotal) : 1;
    ...
    // Semua biaya admin dan program marketplace dihitung dari summary.finalTotal (Omset Bersih) & dibulatkan ke Rupiah utuh
    ...
    try {
        ...
        hideModal();
        alert("Transaksi berhasil disimpan! Semua biaya marketplace telah sinkron.");
...
async function deleteReturnItem(itemToDelete) {
    ...
    try {
        ...
        if (newItems.length === 0) {
            ...
            await updateDoc(returnDocRef, { items: newItems });
        }

        state.retur = state.retur.filter(r => {
            if (r.id !== itemToDelete.returnDocId) return true;
            ...
            <div v-show="uiState.activeSupplierView !== 'form'" class="animate-fade-in-up">
                ...
                <div class="flex flex-wrap justify-between items-center gap-4 mb-8">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button 
    @click="showModal('masterHargaSupplier')" 
    class="bg-indigo-600 text-white px-4 py-2 rounded-lg font-bold hover:bg-indigo-700 flex items-center gap-2 shadow-lg"
>
    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
    </svg>
    Update Master Harga Supplier
</button>
                        <button 
    ...
    <span class="text-xs font-semibold px-2.5 py-1 rounded-full capitalize whitespace-nowrap" :class="{
    ...
    <button 
        @click="showModal('viewPurchaseOrder', filteredPurchaseOrders.find(o => o.id === item.orderId))" 
